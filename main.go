...
func PlotSVG(f func(x, y float64) float64, w io.Writer) {
	zmin, zmax := minmax(f)
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			...
			fmt.Fprintf(w, "<polygon style='stroke: %s' points='%g,%g %g,%g %g,%g %g,%g'/>\n",
				color(f, i, j, zmin, zmax), ax, ay, bx, by, cx, cy, dx, dy)
		}
	}
	fmt.Fprintln(w, "</svg>")
}
...
func color(f func(x, y float64) float64, i, j int, zmin, zmax float64) string {
	min := math.NaN()
	max := math.NaN()
	for xoff := 0; xoff <= 1; xoff++ {
		for yoff := 0; yoff <= 1; yoff++ {
